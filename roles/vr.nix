{ nixpkgs, inputs, pkgs, config, ... }:
{
  ...
  services.udev.extraRules = ''
    ...
    SUBSYSTEM=="usb", ATTRS{idVendor}=="045e", ATTRS{idProduct}=="065d", MODE="0666", GROUP="users", TAG+="uaccess"
  
    # WMR HID devices
    KERNEL=="hidraw*", ATTRS{idVendor}=="03f0", ATTRS{idProduct}=="0580", MODE="0666", GROUP="users"
    KERNEL=="hidraw*", ATTRS{idVendor}=="045e", ATTRS{idProduct}=="065d", MODE="0666", GROUP="users"
  '';
}
## Keeping this around for reference, this was the old way of doing it
# let
#   # overlay = final: prev: {
